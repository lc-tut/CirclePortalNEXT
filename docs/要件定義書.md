...
| `PUT` | **/circles/{id}** | **代表者 / 管理者** | サークル情報を更新する。<br>※代表者は「自分のサークル」のみ操作可能。 |
| `DELETE` | **/circles/{id}** | **代表者 / 管理者** | サークルを削除する（論理削除）。 |
...
### 4.1. 共通定義 (Enum)
  ...
  * **Campus:** `八王子`, `蒲田`
  * **CircleCategory:** `運動系`, `文化系`, `委員会`
  * **SysRole (Global):** `SystemAdmin`, `General`
  * **CircleRole (Local):** `Leader`, `Member`
...
サークル個別の役割とは別に、システム全体における権限を管理するカラムがある。
  ...
  * `username`: String
  * `email`: String (Unique)
  * `sys_role`: Enum(SysRole) (AdminかGeneralか)
...
erDiagram
    ...
    Users {
        UUID id PK "Keycloak IDと共通化可能"
        string username
        string email UK
        string auth_user_id UK "Keycloakのsub (UUID v4) - Nullable"
        datetime created_at
    }
